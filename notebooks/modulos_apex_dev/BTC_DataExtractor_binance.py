...
class ExtractorDatosBinance:
    def __init__(self, symbol='BTCUSDT', timeframe=Client.KLINE_INTERVAL_1DAY, ventana_critica=5):
        # Cliente público (sin API Keys porque solo vamos a leer datos)
        self.client = Client()
        self.symbol = symbol
    ...
    def obtener_datos(self, start_date, end_date, buffer_dias=40):
        ...
        # python-binance acepta strings legibles
        str_fetch = dt_fetch_real.strftime('%Y-%m-%d')
        str_end = pd.to_datetime(end_date).strftime('%Y-%m-%d')
        
        print(f"[Binance Native] Descargando {self.symbol} desde {str_fetch}...")
        
        # 2. Extracción directa (la librería maneja la paginación internamente)
        velas = self.client.get_historical_klines(
            self.symbol, 
            self.timeframe, 
            str_fetch, 
            str_end
        ...
        # 3. Binance devuelve 12 columnas, solo necesitamos las primeras 6
        ...
        # 4. Construcción y limpieza para que sea idéntico al de CCXT
